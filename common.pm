...
#$Char::my_name = "Bulrathi"; !!!брать функцией из доп.файла
#$U::mode = "AC";
...
$timeoffset = 11; #Разница по времени между слосом и локальной машиной
...
#hook {sendl("ask gossiper about gossips");} "tick";
...
#bindkey { sendl('recite recall') } "f1";        #  F1
#bindkey { sendl("get recall $U::recall_container\r\nrecite recall") } "f2";
#bindkey { sendl("qua $U::potion") } "f3";
#bindkey { sendl($U::F4_cmd) } "268";
#bindkey { sendl("o followers assi $Char::my_name") } "269";
#bindkey { sendl("o followers kill $U::target") } "270";
...
#Первый бот будет для чопа
#Структура такая:
#1. trigger на damnation или curse - начинаем autoblind (ставится триггер№2)
#2. trigger на blind - начинаем драться
#3. в этом же триггере включается триггер на
# $U::tank [Lvl:%1  %2hp %3ma %4mv] - если надо лечить
# он посылает rtt;gg если не надо trip;gg
# в этом же триггере если танк дохнет - $U::tank переставляется на 2.танка
# тут же снимается триггер на блинд
# тут же ставиться триггер на получение экспы
#4. триггер на получение экспы - выключается №3 включается №1
